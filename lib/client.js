...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var cssText =
      ...
      // selectable rows / nav items that use the pure photo accent. Do not
      // apply this to tabs: Settings tabs keep the dark panel background, so
      // an on-accent (often dark) label would disappear there.
      "html[data-dsh-skin-bg=image] [aria-selected=\"true\"]:not([role=\"tab\"]),html[data-dsh-skin-bg=image] [class*=\"selected\"]:not([role=\"tab\"]):not(.dsh-skins-card){color:var(--dsh-skin-on-accent,#fff)!important}" +
      // Selected session/project rows keep the PURE accent background (that is
      // the signature look), overriding the translucent hover well — the label
      // rule above flips the text to the accent-readable pole.
      ...
      // video mode shares the same on-accent / selected-row rules as image mode
      "html[data-dsh-skin-bg=video] [aria-selected=\"true\"]:not([role=\"tab\"]),html[data-dsh-skin-bg=video] [class*=\"selected\"]:not([role=\"tab\"]):not(.dsh-skins-card){color:var(--dsh-skin-on-accent,#fff)!important}" +
              ...
              if (isVideoBg) {
                ...
                idbLoadBlob().then(function (blob) {
                  if (blob) {
                    ...
                    applyVideoBackground(blobUrl, savedCustom.veil || 0.25, currentAppearance());
                  }
                }).catch(function (err) {
            ...
            if (id === "skin-custom") {
              ...
              if (image === "video") {
                ...
                idbLoadBlob().then(function (blob) {
                  if (blob) {
                    var blobUrl = URL.createObjectURL(blob);
                    applyVideoBackground(blobUrl, customSeeds.veil, currentAppearance());
                  }
                }).catch(function (err) {

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Dual-version support: DSH up to 0.1.1 seeds `@deepseek-ai/dsh-client-runtime`
    // (the require above targets its /client export); the alpha line split that
    // package and seeds `@deepseek-ai/dsh-client-store` instead. Only defineStore
    // is consumed, and its decl/handle contract is byte-identical on both
    // platforms, so whichever branch lands behaves the same.
    var clientRuntime;
    try {
      clientRuntime = require("@deepseek-ai/dsh-client-runtime/client");
    } catch (errRuntime) {
      try {
        clientRuntime = require("@deepseek-ai/dsh-client-store");
      } catch (errStore) {
        throw new Error("[dsh-client-ui-skins] no compatible client platform: need @deepseek-ai/dsh-client-runtime/client (DSH <= 0.1.1) or @deepseek-ai/dsh-client-store (DSH alpha) in the module table");
      }
    }
     ...
     */
    function deriveTokens(seeds, translucent, appearance, inputAlpha, backgroundAlpha) {
      ...
      // label-primary-inverted is the readable pole opposite the text colour
      // (dark text -> light mark, light text -> dark mark). button-contrast-fill
      // is the BACKGROUND that inverted label sits on (toasts, remove buttons),
      // so it must be the pole opposite the inverted label — NOT the accent.
      // Deriving it from the accent let a light-accent skin pair a dark fill
      // with the (always-dark in dark mode) inverted label, hiding toast text.
      var invertedLabel = readableOn(text);
      var contrastFill = readableOn(invertedLabel);
      ...
      // deep translucent.
      // The background control governs the page wash itself as well as the
      // wallpaper veil.  A zero value must be genuinely transparent; using a
      // hidden fixed 0.3/0.4 wash here made the first half of the slider look
      // like it did nothing.
      var wa = backgroundAlpha == null ? (dark ? 0.3 : 0.4) : backgroundAlpha;
      var wash = translucent ? (dark ? "rgba(14, 16, 22, " + wa + ")" : "rgba(250, 250, 252, " + wa + ")") : null;
      ...
      // Labels on a photograph must not be pre-composited against the
      // synthetic `surface`: the pixels behind them are the actual image and
      // can be much lighter or darker. Use opaque, contrast-checked steps
      // for custom image/video skins instead. This keeps metadata, tool labels
      // and sidebar text readable at low background-mask values without making
      // the wallpaper itself any less transparent.
      var labelSecondary;
      var labelTertiary;
      var labelCaption;
      var labelDimmed;
      if (translucent) {
        labelSecondary = dark ? "#dfe5ef" : "#292f39";
        labelTertiary = dark ? "#c5cdd9" : "#444d5a";
        labelCaption = dark ? "#a9b3c1" : "#5c6775";
        labelDimmed = dark ? "#8f9baa" : "#727d8b";
      } else {
        labelSecondary = alphaOver(text, dark ? 0.78 : 0.72, surface);
        labelTertiary = alphaOver(text, dark ? 0.58 : 0.55, surface);
        labelCaption = alphaOver(text, dark ? 0.48 : 0.42, surface);
        labelDimmed = alphaOver(text, dark ? 0.38 : 0.32, surface);
      }
      ...
      // Input well alpha is user-tunable via the 输入框不透明度 slider.
      // Keep this as alpha, rather than pre-compositing a solid colour, so a
      // 0% value genuinely lets the wallpaper show through.
      var ia = inputAlpha == null ? (dark ? 0.22 : 0.12) : inputAlpha;
      ...
      var tokens = {
        ...
        // contrast-fill is the background for the inverted label (toasts,
        // remove buttons), so it must contrast with the inverted label — the
        // pole opposite it — not with the accent.
        ...
        // -- interactions --------------------------------------------------
        // translucent mode: hover wells stay translucent so the photo shows
        // through AND text stays readable on them — pure accent here would
        // wash out labels on every hover surface (warnings, files, buttons).
        // Selected rows get the pure accent via a targeted rule below.
        "--dsw-alias-interactive-bg-hover": translucent ? rgba(accent, dark ? 0.28 : 0.14) : hoverBg,
        "--dsw-alias-interactive-bg-active": translucent ? rgba(accent, dark ? 0.38 : 0.2) : activeBg,
        "--dsw-alias-interactive-bg-hover-solid": translucent ? rgba(accent, dark ? 0.3 : 0.16) : layer2,
        "--dsw-alias-interactive-bg-hover-accent": translucent ? rgba(accent, dark ? 0.5 : 0.28) : hoverAccent,
        ...
        // Input wells are real translucent layers. The slider controls the
        // CSS alpha directly: 0% = wallpaper fully visible; 100% = opaque.
        "--dsw-specific-input-major": translucent ? rgba(dark ? "#0e1016" : "#fafafc", ia) : layer1,
        "--dsw-specific-login-input": translucent ? rgba(dark ? "#0e1016" : "#fafafc", ia) : layer2,
    ...
    // =====================================================================
    // video wallpaper: IndexedDB blob store + <video> background layer
    // =====================================================================
    var IDB_DB = "dsh-skins";
    var IDB_STORE = "videos";
    var IDB_KEY = "custom";
    var CUSTOM_LS_VIDEO = "dsh-skins.custom.video"; // "1" when a video bg is active
    ...
    /**
     * Decode a video file's first frame onto a canvas, then reuse the same
     * colour-sampling logic as processImage to derive seeds. The video Blob
     * is stored separately in IndexedDB; this function only returns seeds.
     * @param file - user-selected video file (mp4/webm).
     * @returns Promise<{ seeds, veil }>.
     */
    function processVideo(file) {
      return new Promise(function (resolve, reject) {
        ...
        video.onloadeddata = function () {
          try {
            // seek to 1s (or 0 if shorter) to land on a representative frame
            var seekTo = Math.min(1, (video.duration || 1) * 0.1);
            video.currentTime = seekTo;
          } catch (err) {
            cleanup();
            reject(new Error("video seek failed: " + err.message));
          }
        };
        video.onseeked = function () {
          try {
            var w = video.videoWidth, h = video.videoHeight;
            if (!w || !h) { cleanup(); reject(new Error("bad video dimensions")); return; }

            // draw the frame to a canvas for colour sampling (64px grid,
            // same as processImage). We do NOT use this canvas as the
            // background — the <video> element itself plays live.
            var size = 64;
            var sample = document.createElement("canvas");
            sample.width = size; sample.height = size;
            var sctx = sample.getContext("2d", { willReadFrequently: true });
            ...
            var seeds, veil;
            if (pixels.length === 0) {
              // degenerate: fall back to frame average
              var fr = 0, fg = 0, fb = 0, n = 0;
              for (var k = 0; k < data.length; k += 4) {
                if (data[k + 3] / 255 < 0.5) continue;
                fr += data[k]; fg += data[k + 1]; fb += data[k + 2]; n++;
              }
              var avg = rgbToHex(fr / n, fg / n, fb / n);
              seeds = { accent: avg, secondary: avg, surface: avg, text: luminance(avg) < 0.4 ? "#f2f4f8" : "#16181d" };
              veil = 0.45;
            } else {
              var sumR = 0, sumG = 0, sumB = 0;
              for (var j = 0; j < pixels.length; j++) { sumR += pixels[j].r; sumG += pixels[j].g; sumB += pixels[j].b; }
              var surface = rgbToHex(sumR / pixels.length, sumG / pixels.length, sumB / pixels.length);
              var buckets = {};
              var pool = saturated.length >= pixels.length * 0.05 ? saturated : pixels;
              for (var m = 0; m < pool.length; m++) {
              ...
              if (best && best.pixels.length >= pool.length * 0.04) {
                var ar = 0, ag = 0, ab = 0;
                for (var q = 0; q < best.pixels.length; q++) { ar += best.pixels[q].r; ag += best.pixels[q].g; ab += best.pixels[q].b; }
                accent = rgbToHex(ar / best.pixels.length, ag / best.pixels.length, ab / best.pixels.length);
              }
              var dark = luminance(surface) < 0.4;
              var text = dark ? "#f2f4f8" : "#16181d";
              ...
              var secondary = hslToHex(aHsl.h + 30, Math.min(85, aHsl.s + 10), Math.max(30, Math.min(80, aHsl.l + 12)));
              var baseLum = luminance(surface);
              veil = baseLum > 0.55 ? 0.2 : baseLum > 0.35 ? 0.28 : 0.38;
              seeds = { accent: accent, secondary: secondary, surface: surface, text: text };
            }

            cleanup();
            resolve({ seeds: seeds, veil: veil });
          } catch (err) {
            cleanup();
            reject(err);
          }
    ...
    // The live <video> element for wallpaper playback, lazily created.
    var videoBgEl = null;
    var videoBgUrl = null;

    /** Create or reuse the fixed <video> background layer. */
    function applyVideoBackground(blobUrl, veil, appearance) {
      var host = document.documentElement;
      if (blobUrl) {
        if (!videoBgEl) {
          ...
          videoBgEl.setAttribute("aria-hidden", "true");
          videoBgEl.style.cssText = "position:fixed;inset:0;width:100%;height:100%;object-fit:cover;z-index:-2;pointer-events:none";
          document.body.appendChild(videoBgEl);
        }
        if (videoBgUrl !== blobUrl) {
          videoBgEl.src = blobUrl;
          videoBgUrl = blobUrl;
        }
        videoBgEl.play().catch(function () { /* autoplay may need a user gesture; loop will retry */ });
        host.style.setProperty("--dsh-skin-bg-veil", String(veil));
        host.style.setProperty("--dsh-skin-bg-mask", appearance === "light" ? "rgba(245, 246, 250, var(--dsh-skin-bg-veil, 0.25))" : "rgba(5, 6, 9, var(--dsh-skin-bg-veil, 0.25))");
        host.setAttribute("data-dsh-skin-bg", "video");
      } else {
        clearVideoBackground();
      }
    }
    /** Remove the <video> background layer and release its object URL. */
    function clearVideoBackground() {
      if (videoBgEl) {
        videoBgEl.pause();
        videoBgEl.removeAttribute("src");
        videoBgEl.load();
        if (videoBgEl.parentNode) videoBgEl.parentNode.removeChild(videoBgEl);
        videoBgEl = null;
      }
      if (videoBgUrl) {
        URL.revokeObjectURL(videoBgUrl);
        videoBgUrl = null;
      }
      var host = document.documentElement;
      host.style.removeProperty("--dsh-skin-bg-veil");
    ...
    var zh = {
      "skins.title": "皮肤",
      "skins.default": "默认",
      "skins.custom": "自定义（选图/视频）",
      "skins.customActive": "自定义",
      "skins.builtin": "内置皮肤",
      "skins.bgOpacity": "背景遮罩",
      "skins.inputOpacity": "输入框不透明度",
      "skins.frosted": "正文加强"
    };
    var en = {
      "skins.title": "Skins",
      "skins.default": "Default",
      "skins.custom": "Custom (image/video)",
      "skins.customActive": "Custom",
      "skins.builtin": "Built-in skins",
      "skins.bgOpacity": "Background veil",
      "skins.inputOpacity": "Input opacity",
      "skins.frosted": "Emphasize body text"
    };
    ...
    var cssText =
      ...
      ".dsh-skins-opacity{display:flex;flex-direction:column;gap:8px;margin-top:6px}" +
      ".dsh-skins-opacity-row{display:flex;align-items:center;gap:8px;font-size:12px;color:var(--dsw-alias-label-secondary)}" +
      ".dsh-skins-opacity-label{min-width:84px;color:var(--dsw-alias-label-tertiary)}" +
      ".dsh-skins-opacity-row input[type=range]{flex:1;accent-color:var(--dsw-alias-brand-primary)}" +
      ".dsh-skins-opacity-val{min-width:36px;text-align:right;color:var(--dsw-alias-label-tertiary);font-variant-numeric:tabular-nums}" +
      ".dsh-skins-frosted-row{display:flex;align-items:center;gap:8px;cursor:pointer;font-size:12px;color:var(--dsw-alias-label-secondary);margin-top:2px}.dsh-skins-frosted-row input[type=checkbox]{accent-color:var(--dsw-alias-brand-primary);width:14px;height:14px}.dsh-skins-frosted-label{color:var(--dsw-alias-label-tertiary)}" +
      "html[data-dsh-skin-bg=image] [role=\"treeitem\"][aria-selected=\"true\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      "html[data-dsh-skin-bg=image] .YDXeBa_sessionRow.YDXeBa_selected,html[data-dsh-skin-bg=image] .YDXeBa_projectRow.YDXeBa_selected{background:var(--dsw-alias-brand-primary,#fff)!important}" +
      "html[data-dsh-skin-bg=image] .dsh-skins-card{color:var(--dsw-alias-label-primary,#eef1f6)!important}" +
      "html[data-dsh-skin-bg=image] [role=\"tab\"][aria-selected=\"true\"]{color:var(--dsw-alias-label-primary,#eef1f6)!important}" +
      // The appearance selector cubes (浅色/深色/跟随系统) use native light or
      // dark backgrounds. A photo can corrupt the normal text token, so this
      // must be an explicit contrast pair rather than a skin-derived token.
      "html[data-dsh-skin-bg=image] body:not([data-ds-dark-theme]) [class*=\"themeCube\"][class*=\"selected\"]{color:#1a1d24!important}" +
      "html[data-dsh-skin-bg=image] body[data-ds-dark-theme] [class*=\"themeCube\"][class*=\"selected\"]{color:#f5f5f5!important}" +
      // Produced-files rows (产物) sit on the pure-accent interactive hover
      // background; their label must flip to the accent-readable pole so the
      // file names stay legible (red -> white, yellow -> dark).
      "html[data-dsh-skin-bg=image] [class*=\"P4kPIW_file\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      // The composer notice / warning bar (uV2eYG_notice) also sits on the
      // pure-accent hover background; flip its text to the accent-readable
      // pole so warnings stay legible in both appearances.
      "html[data-dsh-skin-bg=image] [class*=\"uV2eYG_notice\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      ...
      // video wallpaper: the <video> element sits at z-index:-2 (set inline);
      // this veil pseudo-element sits at z-index:-1 above it, same as image mode.
      "html[data-dsh-skin-bg=video]::after{content:'';position:fixed;inset:0;z-index:-1;background:var(--dsh-skin-bg-mask, rgba(5,6,9,0.3))}" +
      "html[data-dsh-skin-bg=video] [role=\"treeitem\"][aria-selected=\"true\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      "html[data-dsh-skin-bg=video] .YDXeBa_sessionRow.YDXeBa_selected,html[data-dsh-skin-bg=video] .YDXeBa_projectRow.YDXeBa_selected{background:var(--dsw-alias-brand-primary,#fff)!important}" +
      "html[data-dsh-skin-bg=video] .dsh-skins-card{color:var(--dsw-alias-label-primary,#eef1f6)!important}" +
      "html[data-dsh-skin-bg=video] [role=\"tab\"][aria-selected=\"true\"]{color:var(--dsw-alias-label-primary,#eef1f6)!important}" +
      "html[data-dsh-skin-bg=video] body:not([data-ds-dark-theme]) [class*=\"themeCube\"][class*=\"selected\"]{color:#1a1d24!important}" +
      "html[data-dsh-skin-bg=video] body[data-ds-dark-theme] [class*=\"themeCube\"][class*=\"selected\"]{color:#f5f5f5!important}" +
      "html[data-dsh-skin-bg=video] [class*=\"P4kPIW_file\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      "html[data-dsh-skin-bg=video] [class*=\"uV2eYG_notice\"]{color:var(--dsh-skin-on-accent,#fff)!important}" +
      // Frosted scrim ONLY under assistant markdown text. The photo can wash
      // out body text where it crosses a highlight; a light frosted layer
      // (blur + translucent surface tint) keeps prose legible without dimming
      // the wallpaper behind the rest of the UI. Scoped to assistant steps so
      // it never tints user bubbles, tabs or the composer. Gated by
      // html[data-dsh-skin-frosted] — the 磨砂效果 switch toggles this attr.
      "html[data-dsh-skin-frosted][data-dsh-skin-bg=image] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5," +
      "html[data-dsh-skin-frosted][data-dsh-skin-bg=video] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5{backdrop-filter:blur(8px) saturate(1.1);-webkit-backdrop-filter:blur(8px) saturate(1.1);background:rgba(255,255,255,0.45);border-radius:8px;padding:2px 6px;margin:-2px -6px}" +
      // dark appearance: a dark translucent tint instead, so light prose stays
      // legible over bright photo regions.
      "html[data-dsh-skin-frosted][data-dsh-skin-bg=image] body[data-ds-dark-theme] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5," +
      "html[data-dsh-skin-frosted][data-dsh-skin-bg=video] body[data-ds-dark-theme] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5{background:rgba(14,16,22,0.42)}";
    ...
    function SkinsRow(props) {
      ...
      var setFrosted = props.setFrosted;
      var activeId = useStore(function (s) { return s.active; });
      var customSeeds = useStore(function (s) { return s.customSeeds; });
      var bgOpacity = useStore(function (s) { return s.bgOpacity; });
      var inputOpacity = useStore(function (s) { return s.inputOpacity; });
      var frosted = useStore(function (s) { return s.frosted; });
      ensureStyle();
      ...
      var customCard = jsxRuntime.jsx(
        "button",
        {
          ...
          onClick: function () {
            ...
            // Safari Web Apps can discard the change event for a detached
            // file input. Keep this short-lived control in the document until
            // the picker completes, then remove it without affecting layout.
            ...
            input.onchange = function () {
              ...
              if (!f) { removeInput(); return; }
              var isVideo = f.type.indexOf("video/") === 0;
              if (isVideo) {
                processVideo(f).then(function (result) {
                  // store the blob in IndexedDB, then apply with the video flag
                  idbSaveBlob(f).then(function () {
                    applySkin("skin-custom", result.seeds, "video", result.veil);
                    removeInput();
                  }).catch(function (err) {
                    removeInput();
                    console.error("[dsh-skins] video storage failed", err);
                  });
                }).catch(function (err) {
                  removeInput();
                  console.error("[dsh-skins] custom video processing failed", err);
                });
              } else {
                processImage(f).then(function (result) {
                  ...
                  removeInput();
                  console.error("[dsh-skins] custom image processing failed", err);
                });
              }
            };
      ...
      // opacity sliders — only for custom image/video skins
      var opacityBlock = customActive ? jsxRuntime.jsxs("div", { className: "dsh-skins-opacity", children: [
            jsxRuntime.jsxs("label", { className: "dsh-skins-opacity-row", children: [
              jsxRuntime.jsx("span", { className: "dsh-skins-opacity-label", children: t("skins.bgOpacity") }),
              jsxRuntime.jsx("input", {
                type: "range", min: 0, max: 100, value: bgOpacity,
                onChange: function (e) { setOpacity("bg", Number(e.target.value)); }
              }),
              jsxRuntime.jsx("span", { className: "dsh-skins-opacity-val", children: bgOpacity + "%" })
            ]}),
            jsxRuntime.jsxs("label", { className: "dsh-skins-opacity-row", children: [
              jsxRuntime.jsx("span", { className: "dsh-skins-opacity-label", children: t("skins.inputOpacity") }),
              jsxRuntime.jsx("input", {
                type: "range", min: 0, max: 100, value: inputOpacity,
                onChange: function (e) { setOpacity("input", Number(e.target.value)); }
              }),
              jsxRuntime.jsx("span", { className: "dsh-skins-opacity-val", children: inputOpacity + "%" })
            ]})
          ]}) : null;

      // 正文加强 (frosted scrim) toggle — shown after the hint text, only for
      // custom image/video skins
      var frostedBlock = customActive ? jsxRuntime.jsxs("label", { className: "dsh-skins-frosted-row", children: [
            jsxRuntime.jsx("input", {
              type: "checkbox", checked: frosted,
              onChange: function (e) { setFrosted(e.target.checked); }
            }),
            jsxRuntime.jsx("span", { className: "dsh-skins-frosted-label", children: t("skins.frosted") })
          ]}) : null;
    ...
    function createSkinsRowStore() {
      return clientRuntime.defineStore({
        init: function () {
          return { active: "system", revision: -1, customSeeds: null, bgOpacity: 25, inputOpacity: 22, frosted: false };
        },
        actions: {
          sync: function (d, active, revision, customSeeds, bgOpacity, inputOpacity, frosted) {
            if (revision <= d.revision) return;
            d.active = active;
            d.revision = revision;
            if (customSeeds !== undefined) d.customSeeds = customSeeds;
            if (bgOpacity !== undefined) d.bgOpacity = bgOpacity;
            if (inputOpacity !== undefined) d.inputOpacity = inputOpacity;
            if (frosted !== undefined) d.frosted = frosted;
          }
        }
      });
    }
    ...
    // user-tunable opacities for custom-image/video skins (0..100)
    var LS_BG_OPACITY = "dsh-skins.bgOpacity";      // background veil strength
    var LS_INPUT_OPACITY = "dsh-skins.inputOpacity"; // input well opacity
    // frosted-scrim toggle for assistant markdown (default on; "0" = off)
    var LS_FROSTED = "dsh-skins.frosted";

    /** Whether the assistant-text frosted scrim is enabled (default on). */
    function frostedEnabled() {
      // default OFF: "1" only when the user explicitly turned it on
      return loadLS(LS_FROSTED, "0") === "1";
    }
    /** Apply/remove the html[data-dsh-skin-frosted] gate that the scrim CSS keys off. */
    function applyFrostedGate() {
      if (typeof document === "undefined") return;
      if (frostedEnabled()) document.documentElement.setAttribute("data-dsh-skin-frosted", "");
      else document.documentElement.removeAttribute("data-dsh-skin-frosted");
    }
    ...
    /** Read a 0..100 opacity from localStorage, clamped; fallback when unset/invalid. */
    function loadOpacity(key, fallback) {
      var raw = loadLS(key, null);
    ...
    function apply(ctx) {
      ...
      ensureStyle();
      // frosted-scrim gate: reflect the persisted toggle on <html> up front
      applyFrostedGate();
      ...
      function resolveSkinTokens(id, appearance, inputAlpha, backgroundAlpha) {
        var app = appearance || currentAppearance();
        if (id === "skin-custom") {
          if (!customSeeds) return null;
          return deriveTokens(customSeeds, true, app, inputAlpha, backgroundAlpha);
        }
      ...
      /** Resolve the custom background veil from its persisted slider value. */
      function effectiveBgVeil() {
        var sampledOpacity = Math.round(((customSeeds && customSeeds.veil) || 0.25) * 100);
        return loadOpacity(LS_BG_OPACITY, sampledOpacity) / 100;
      }

      /** Reflect the current skin into the settings-row store. */
      function publish() {
        revision += 1;
        var bgOp = currentSkin === "skin-custom" ? Math.round(effectiveBgVeil() * 100) : 25;
        var inOp = isOurSkin(currentSkin) ? loadOpacity(LS_INPUT_OPACITY, 22) : 22;
        bound && bound.sync(isOurSkin(currentSkin) ? currentSkin : "system", revision, customSeeds, bgOp, inOp, frostedEnabled());
      }
      ...
      function applySkinState() {
        ...
        // custom skins: honour the user-tunable input opacity (0..1)
        var inputAlpha = null;
        if (currentSkin === "skin-custom") {
          inputAlpha = loadOpacity(LS_INPUT_OPACITY, 22) / 100;
        }
        var backgroundAlpha = currentSkin === "skin-custom" ? effectiveBgVeil() : null;
        var tokens = resolveSkinTokens(currentSkin, appearance, inputAlpha, backgroundAlpha);
        if (tokens) {
        ...
        // custom background: re-tint the mask with the appearance (bg unchanged).
        // The veil strength honours the user-tunable bg opacity (falls back to
        // the colour-sampled veil when unset).
        if (currentSkin === "skin-custom" && customSeeds) {
          var isVideoBg = loadLS(CUSTOM_LS_VIDEO, "") === "1";
          var bgVeil = effectiveBgVeil();
          if (isVideoBg) {
            // video background: the <video> element persists across repaints;
            // just refresh the veil mask for the new appearance.
            if (videoBgEl) {
              document.documentElement.style.setProperty("--dsh-skin-bg-veil", String(bgVeil));
              document.documentElement.style.setProperty("--dsh-skin-bg-mask", appearance === "light" ? "rgba(245, 246, 250, var(--dsh-skin-bg-veil, 0.25))" : "rgba(5, 6, 9, var(--dsh-skin-bg-veil, 0.25))");
            }
          } else {
            var img = loadLS(CUSTOM_LS_IMAGE, "");
            if (img) applyBackground(img, bgVeil, appearance);
          }
        }
      }
        ...
        if (saved === "skin-custom") {
          try {
            ...
            if (savedCustom && savedCustom.accent) {
              ...
              var isVideoBg = loadLS(CUSTOM_LS_VIDEO, "") === "1";
              if (isVideoBg) {
                // async: load the video Blob from IndexedDB, then start playback
                idbLoadBlob().then(function (blob) {
                  if (blob) {
                    var blobUrl = URL.createObjectURL(blob);
                    applyVideoBackground(blobUrl, effectiveBgVeil(), currentAppearance());
                  }
                }).catch(function (err) {
                  console.error("[dsh-skins] video restore failed", err);
                });
              } else {
                var savedImage = loadLS(CUSTOM_LS_IMAGE, "");
                if (savedImage) applyBackground(savedImage, effectiveBgVeil(), currentAppearance());
              }
            }
        ...
        return {
          applySkin: function (id, seeds, image, veil) {
            if (id === "skin-custom") {
              ...
              if (image === "video") {
                // video wallpaper: the Blob is already in IndexedDB; create
                // an object URL for playback and mark the video flag.
                saveLS(CUSTOM_LS_IMAGE, "");
                saveLS(CUSTOM_LS_VIDEO, "1");
                // clear any lingering static-image background
                applyBackground(null);
                // the blob URL was created in the upload handler; if we are
                // restoring, idbLoadBlob already made one — but on fresh
                // upload we need to create it here from the stored blob.
                idbLoadBlob().then(function (blob) {
                  if (blob) {
                    var blobUrl = URL.createObjectURL(blob);
                    applyVideoBackground(blobUrl, effectiveBgVeil(), currentAppearance());
                  ...
                  console.error("[dsh-skins] video apply failed", err);
                });
              } else if (image) {
                // static image: clear video, apply image background
                saveLS(CUSTOM_LS_VIDEO, "");
                clearVideoBackground();
          ...
          setOpacity: function (kind, value) {
            var v = Math.max(0, Math.min(100, value));
            saveLS(kind === "bg" ? LS_BG_OPACITY : LS_INPUT_OPACITY, String(v));
            if (kind === "bg") {
              // The page base is another translucent wash derived from the
              // same control. Updating only ::after left that old wash in
              // place, so 0% still looked dimmed by the sampled image skin.
              // Repaint both surfaces from the persisted value.
              applySkinState();
            } else {
              // input opacity: re-derive tokens so the wells repaint
              applySkinState();
            }
            publish();
          },
          setFrosted: function (on) {
            saveLS(LS_FROSTED, on ? "1" : "0");
            applyFrostedGate();
            publish();
          }
        };

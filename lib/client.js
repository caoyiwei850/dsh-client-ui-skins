...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      "skins.customHint": "选一张 PNG / JPG / WebP 或 MP4 / WebM 视频，整套配色跟着画面走",
    ...
    var en = {
      ...
      "skins.customHint": "Pick a PNG / JPG / WebP or MP4 / WebM video — the whole palette follows it",
    ...
    var cssText =
      ...
      // it never tints user bubbles, tabs or the composer.
      "html[data-dsh-skin-bg=image] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5," +
      "html[data-dsh-skin-bg=video] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5{backdrop-filter:blur(8px) saturate(1.1);-webkit-backdrop-filter:blur(8px) saturate(1.1);background:rgba(255,255,255,0.45);border-radius:8px;padding:2px 6px;margin:-2px -6px}" +
      // dark appearance: a dark translucent tint instead, so light prose stays
      // legible over bright photo regions.
      "html[data-dsh-skin-bg=image] body[data-ds-dark-theme] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5," +
      "html[data-dsh-skin-bg=video] body[data-ds-dark-theme] [data-chat-flow-kind=assistant-step] ._markdown_1r4m5_5{background:rgba(14,16,22,0.42)}";
    ...
    function createSkinsRowStore() {
      return clientRuntime.defineStore({
        init: function () {
          return { active: "system", revision: -1, customSeeds: null, bgOpacity: 25, inputOpacity: 22 };
        },
        actions: {
          sync: function (d, active, revision, customSeeds, bgOpacity, inputOpacity) {
    ...
    function apply(ctx) {
      ...
      function publish() {
        ...
        bound && bound.sync(isOurSkin(currentSkin) ? currentSkin : "system", revision, customSeeds, bgOp, inOp);
      }

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var cssText =
      ...
      // The appearance selector cubes (浅色/深色/跟随系统) keep their native
      // selected background (bg-module-platform — near-black in dark mode,
      // pale in light mode). Their label must follow the appearance, not the
      // photo accent: dark appearance -> white label on the dark cube, light
      // appearance -> dark label on the pale cube.
      "html[data-dsh-skin-bg=image] [class*=\"themeCube\"][class*=\"selected\"]{color:var(--dsh-skin-brand-mark,#fff)!important}" +
    ...
    function apply(ctx) {
      ...
      function currentAppearance() {
        // The active skin can advertise the colour scheme of the sampled
        // image, which is independent from DSH's appearance switch.  The
        // document marker is the live source of truth: a light photo used in
        // DSH dark mode must still receive the dark palette and light text.
        if (typeof document !== "undefined") {
          if (document.body && document.body.hasAttribute("data-ds-dark-theme")) return "dark";
          // DSH removes this marker in light mode.  Do not inspect
          // `color-scheme`: a custom image skin can set that independently.
